...
class LTVCalculator:
    def __init__(self, base_df, date_order_column, date_register_column, cumsum_base_column, ltv_index_columns, ltv_type='Q'):
    ...
    def execute(self):
        ...
        self.ltv_df = self.base_df.pivot_table(index=self.index_column, columns=self.ltv_index_columns, values=self.cumsum_base_column,
                             aggfunc=np.sum).cumsum().round(2)

        return self.base_df

    def pd_to_csv(self, filename, decimal=','):
        if self.ltv_df is None:
            raise AttributeError('LTV DataFrame not defined, please launch execute before')
        self.ltv_df.fillna('').to_csv(filename, index=False, decimal=decimal)